 ...
 *
 * Converted to JSX for the Wowstack (Vite + CSS Modules) codebase — logic unchanged.
 */
...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    const paint = (progress) => {
      ...
      // Keep scale on the clip to avoid text paint limits.
      ...
      // Reveal the content as soon as the camera has filled the frame (t reaches
      // 1 at p≈0.78), so it rises into view instead of leaving an empty beat.
      section.style.setProperty("--gp-reveal", String(isStatic ? 1 : smooth(0.78, 0.84, p)));
